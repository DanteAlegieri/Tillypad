...
## Restaurant OS 3.1 — цифровые помощники

Главное изменение: навигация больше не строится вокруг
обычных отчётов и модулей.

Появилась цифровая команда:
...
BI-панель сохранена и стала рабочим пространством
Бизнес-аналитика.
...
## Restaurant OS AI 4.1 — AI Director

Добавлено:
- отдельный модуль gateway/app/ai;
- прозрачный Restaurant Health Score;
- факторы и веса индекса;
- серверный Recommendation Engine;
- автоматический брифинг владельца;
- значимые события;
- рекомендации с приоритетом, причиной, доказательствами,
  ожидаемым эффектом и уверенностью;
- кнопка «Почему?» в каждой рекомендации.

Обновлять Agent не требуется.


## Restaurant OS AI 4.2 — Sprint 2.1

Добавлено:

- таблица `health_scores`;
- ежедневная история индекса ресторана;
- динамика индекса по дням;
- сравнение с предыдущим значением;
- прозрачная расшифровка факторов;
- вес каждого фактора;
- объяснение начисленных баллов;
- отдельный API истории индекса.

Новый API:

```text
GET /api/web/{agent_id}/ai/health/history
```
